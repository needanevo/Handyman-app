...
class PricingEngine:
    ...
    def calculate_service_price(
        ...
        if service.pricing_model == PricingModel.FLAT:
            ...
            hours = ai_suggestion.estimated_hours if ai_suggestion else (service.typical_duration / 60.0)
            ...
            if hours < 1.0:
                ...
                billable_hours = float(Decimal(str(hours)).quantize(Decimal('0.25'), rounding=ROUND_HALF_UP))
    ...
    def create_quote_item(
        ...
        if not description:
            ...
            if ai_suggestion and ai_suggestion.suggested_materials:
                description += f" (Materials: {', '.join(ai_suggestion.suggested_materials[:3])})"
    ...
    def estimate_deposit_amount(self, total_amount: float, deposit_percentage: float = 0.25) -> float:
    ...
    def validate_pricing(self, quote_items: List[QuoteItem]) -> List[str]:
        ...
        if total < self.minimum_charge:
            warnings.append(f"Total amount ${total:.2f} is below minimum charge ${self.minimum_charge:.2f}")
        
        # Check for suspiciously high total (over $2000)
        if total > 2000:
            warnings.append(f"Total amount ${total:.2f} is unusually high - please review")
