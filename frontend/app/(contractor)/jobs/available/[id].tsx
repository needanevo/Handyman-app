/**
 * Job Detail Screen — Contractor (Available Jobs View)
 *
 * Displays job details from the available jobs feed.
 * Allows contractors to view full job information before accepting/bidding.
 * Shares identical UI/UX with handyman version for consistent experience.
 */
...
// Feature flag — flip to true when backend accept endpoints are ready
...
export default function ContractorAvailableJobDetail() {
  ...
  const { data: job, isLoading, error } = useQuery({
    queryKey: ['contractor-available-job', id],
    queryFn: async () => {
      const res = await contractorAPI.getJob(id!);
      return res as any;
    },
    enabled: !!id,
  });
  ...
  const acceptJobMutation = useMutation({
    mutationFn: () => contractorAPI.acceptJobAtQuote(id!),
    onSuccess: (data) => {
      ...
      Alert.alert(
        ...
        [
          { text: 'View My Jobs', onPress: () => router.push('/(contractor)/jobs/active') },
  ...
  const submitBidMutation = useMutation({
    mutationFn: () => {
      ...
      return contractorAPI.submitBid(id!, bidData);
    },
    onSuccess: (data) => {
  ...
  const isSubmitting = acceptJobMutation.isPending || submittingBid || submitBidMutation.isPending;
        ...
        {(job.customer_name || job.customer_phone || job.customer_email) && (
          <View style={styles.card}>
            ...
            <View style={styles.customerContactRow}>
              {job.customer_phone && (
                <TouchableOpacity
                  onPress={() => Alert.alert('Phone', job.customer_phone)}
              ...
              {job.customer_email && (
                <TouchableOpacity
                  onPress={() => Alert.alert('Email', job.customer_email)}
                  style={styles.contactItem}
                >
